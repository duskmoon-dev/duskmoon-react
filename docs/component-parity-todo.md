...
Implement the React component surface in `@duskmoon-dev/components` so it reaches
component parity with `/Users/gao/Workspace/zdns/z-design`, while using
DuskMoon design tokens and DuskMoon UI component styles as the visual source of
truth.
...
- Current repo: `/Users/gao/Workspace/duskmoon-dev/duskmoon-react`, commit
  `ecbe9cf`.
- Parity API source: `/Users/gao/Workspace/zdns/z-design`, branch
  `task-XMG-7714-dev`, commit `bf886a5`.
...
- z-design component directories excluding `__tests__`, `assets`, `locales`,
  `provider`, and `utils`: 83.
- Missing component directories in this repo: 1.
- Executable parity status: 105/105 public targets complete.
- z-design starts with `export * from 'antd'`; the local z-design install
  currently resolves Ant Design `5.29.3`, so Ant Design-compatible components are
  also part of the target surface.
...
1. Public API and behavior parity comes from
   `/Users/gao/Workspace/zdns/z-design/src/index.tsx` and each matching
   `src/<component>/index.ts` plus implementation/demo/test files. z-design names
   are source names only; target names follow the DuskMoon prefix policy below.
...
- [ ] Treat z-design and Ant Design as API and behavior references only. Do not
      copy their source code into this repo.
- [ ] Use `Dm` as the DuskMoon public prefix. Do not expose new `Z*` public
      component names from this repo.
- [ ] Map z-design source names to DuskMoon target names:
      `ZLayout`/`z-layout` -> `DmLayout`/`dm-layout`,
      `ZProTable`/`z-pro-table` -> `DmProTable`/`dm-pro-table`,
      `ZMessage`/`z-message` -> `DmMessage`/`dm-message`, and so on.
- [ ] Map exported z-design helpers to DuskMoon names:
      `getZDesignTheme` -> `getDmTheme`,
      `onThemeUpdate` -> `onDmThemeUpdate`,
      `setZDesignPrefixCls` -> `setDmPrefixCls`,
      `setZDesignPrimaryColor` -> `setDmPrimaryColor`, and
      `setZDatePickerLocale` -> `setDmDatePickerLocale`.
- [ ] Keep unprefixed standard component names unprefixed where z-design and the
      DuskMoon API both use generic names: `Button`, `Input`, `Table`, `Modal`,
      `Select`, etc.
...
- [ ] Do not rely on `export * from 'antd'` as the final solution. Own explicit
      root exports, subpath exports, tests, and docs in this repo.

## Architecture Gaps To Resolve First

- [ ] Standardize ref handling before bulk implementation. Current components
      mix React 19 `ref` props and wrapper patterns; z-design wrappers preserve
      refs for many focusable components.
- [ ] Decide provider parity. z-design wraps every component in a configurable
      provider with `prefixCls`, primary color, theme update callbacks, locale, and
      service holder rendering. DuskMoon React currently has `ThemeProvider` and
...
- [ ] Decide locale parity. z-design reads language from cookie/html and has
      per-component locale bundles. DuskMoon React needs a locale story before
      shipping `Dm*` composite components with user-facing text.
- [ ] Decide dependency policy for large interaction models. z-design gets many
      behaviors from Ant Design, ag-grid, `@alifd/next`, `react-intl`, `ahooks`,
      `lodash`, `size-sensor`, and `react-infinite-scroll-component`; DuskMoon React
      must decide case-by-case whether to depend on proven behavior libraries or
...
- [ ] Every public component exported by z-design, including the Ant Design
      re-export surface, has a matching React export in `@duskmoon-dev/components`.
- [ ] Each component has a stable root export and subpath export where parity
      requires one.
- [ ] Existing components are audited against z-design API and DuskMoon visual
      rules, not assumed complete because files already exist.
- [ ] Component props, static members, forwarded refs, service APIs, and compound
      components match the z-design contract unless an intentional difference is
      documented.
- [ ] Styling uses DuskMoon tokens and DuskMoon UI class recipes rather than
      copying z-design/Ant Design visual defaults.
...
- [ ] Decide the adapter policy for Ant-style components: DuskMoon-native first,
...
For each component: compare z-design props/static members, current local
props/classes, DuskMoon core style recipe, tests, and docs.

### Workstream B: Ant Design-Compatible Export Surface

- [ ] Implement or explicitly track every row in
      `docs/component-api-inventory.md#ant-design-compatible-components`.
- [ ] Add Ant Design re-export-only targets that are not already represented by
      z-design wrapper directories: `App`, `BackTop`, `Breadcrumb`,
...
- [ ] Own explicit root exports and package subpaths instead of using
      `export * from 'antd'`.
- [ ] For every Ant Design-compatible component, decide whether implementation is
...
### Workstream D: Standard z-design Surface Without Direct Core Match
...
- [x] `message`
- [x] `notification`
- [x] `dm-message` from z-design `z-message`
- [ ] `modal` static APIs, if z-design parity requires them

### Workstream F: DuskMoon-Prefixed Composite Components

These should come after foundation, data entry, overlays, table, tree, and
navigation primitives are stable. z-design `Z*`/`z-*` components are the parity
source; this repo should expose `Dm*`/`dm-*` targets.

- [x] `dm-auxiliary` from z-design `z-auxiliary`
- [x] `dm-breadcrumb` from z-design `z-breadcrumb`
- [x] `dm-date-picker` from z-design `z-date-picker`
- [x] `dm-drawer` from z-design `z-drawer`
- [x] `dm-infinite-scroll` from z-design `z-infinite-scroll`
- [x] `dm-layout` from z-design `z-layout`
- [x] `dm-menu` from z-design `z-menu`
- [x] `dm-message` from z-design `z-message`
- [x] `dm-page-header` from z-design `z-page-header`
- [x] `dm-pagination` from z-design `z-pagination`
- [x] `dm-pro-table` from z-design `z-pro-table`
- [x] `dm-query` from z-design `z-query`
- [x] `dm-search` from z-design `z-search`
- [x] `dm-splitter` from z-design `z-splitter`
- [x] `dm-status` from z-design `z-status`
- [x] `dm-table` from z-design `z-table`
- [x] `dm-tabs` from z-design `z-tabs`
- [x] `dm-toolbar` from z-design `z-toolbar`
- [x] `dm-tree` from z-design `z-tree`
- [x] `dm-truncate` from z-design `z-truncate`

Composite contracts to preserve or intentionally replace:

- [ ] `DmLayout`, `DmMenu`, `DmBreadcrumb`, `DmAuxiliary`, and `DmDrawer`:
      preserve the behavior of z-design `ZLayout`, `ZMenu`, `ZBreadcrumb`,
      `ZAuxiliary`, and `ZDrawer`: `IMenu` schema, selected/open keys, recent
      breadcrumb storage, HTML auxiliary content, and custom drawer footer/submit
      behavior.
- [x] `usePersistedPageSize`: preserve z-design page-size persistence helper.
- [ ] `DmQuery`, `DmSearch`, `DmTable`, and
      `DmPagination`: preserve z-design schema-driven search items,
      compact/collapsed modes, derived table search items, hidden/order column
      settings, custom pagination, and `_pageSize` persistence.
- [x] `DmProTable` and `dm-pro-table-inner`: preserve z-design column
      transformation, toolbar/column setting behavior, `${persistenceKey}_ag_grid`
      persistence, and the chosen ag-grid/browser compatibility story. Treat
      `dm-pro-table-inner` as an internal implementation target unless explicitly
      promoted later.
- [ ] `DmSplitter`: expose `DmSplitter.Panel` while preserving z-design
      local/network persistence modes, collapse/reset behavior, and resize tests.
...
- [x] `DmDatePicker`: exposes Dm picker wrappers and
      `setZDatePickerLocale` semantics, plus moment/dayjs conversion behavior.

## Cross-Cutting Parity Checks

- [ ] Static members: `Button.Group`, `Typography.*`, `Form.*`, `Input.*`,
      `Select.*`, `Table.*`, `Modal.*`, `FloatButton.*`, and other z-design compound
      exports.
...
- [ ] Locale hooks: map z-design locale behavior to DuskMoon-compatible runtime
      locale support before implementing user-facing strings.
